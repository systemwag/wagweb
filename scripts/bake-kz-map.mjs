/**
 * Bake a static PNG of the Kazakhstan outline for the PDF print engine.
 *
 * Why: react-pdf's SVG renderer is more limited than Chromium's. Rendering
 * the full KZ_MAIN path (~50KB of commands) at PDF time risks rendering
 * artifacts and adds bytes. We render it once with Puppeteer and embed the
 * resulting PNG, which is ~80KB at 300 DPI and renders instantly.
 *
 * Run: node scripts/bake-kz-map.mjs
 * Output: public/portfolio/kz-map.png
 *
 * Re-run only when the KZ_MAIN path changes (i.e. you redraw the map).
 */
...
const OUT = join(ROOT, 'public', 'portfolio', 'kz-map.png');

// Same viewBox geometry as src/components/Map/KazakhstanMap.tsx
const VB_X = -100, VB_Y = 30, VB_W = 1200, VB_H = 820;

// Lifted from KazakhstanMap.tsx
...
<style>
  ...
  #c { width: ${VB_W}px; height: ${VB_H}px; }   /* match SVG viewBox so the PNG has no internal letterbox */
</style>
</head><body>
<svg id="c" viewBox="${VB_X} ${VB_Y} ${VB_W} ${VB_H}" xmlns="http://www.w3.org/2000/svg">
  <g transform="translate(-50,874) scale(0.1,-0.1)">
    <path d="${KZ_MAIN}" fill="rgba(13,18,34,0.6)" stroke="none" />
    <path d="${KZ_MAIN}" fill="none" stroke="rgba(0,196,167,0.7)" stroke-width="14" stroke-linejoin="round" stroke-linecap="round" />
  </g>
</svg>
</body></html>`;

async function main() {
  console.log('[map] Launching headless Chromium...');
  ...
  try {
    ...
    // Render at 2x for crisp output in PDF
    await page.setViewport({ width: VB_W, height: VB_H, deviceScaleFactor: 2 });
    ...
    const buffer = await page.screenshot({
      type: 'png',
      omitBackground: true,
      clip: { x: 0, y: 0, width: VB_W, height: VB_H },
    });
    writeFileSync(OUT, buffer);

    const sizeKb = Math.round(buffer.length / 1024);
    console.log(`[map] OK: ${OUT}  (${sizeKb} KB)`);
  } finally {
    await browser.close();
  }
}
