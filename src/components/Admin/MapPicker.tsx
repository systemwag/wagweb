...
export default function MapPicker({ x, y, onChange }: Props) {
  const svgRef = useRef<SVGSVGElement>(null);

  const handleClick = (e: React.MouseEvent<SVGSVGElement>) => {
    if (!svgRef.current) return;
    const rect = svgRef.current.getBoundingClientRect();
    const nx = Math.round(VB_X + ((e.clientX - rect.left) / rect.width) * VB_W);
    const ny = Math.round(VB_Y + ((e.clientY - rect.top) / rect.height) * VB_H);
    onChange(nx, ny);
  };

  return (
    <div className={styles.wrap}>
      <p className={styles.hint}>Кликните на карте чтобы поставить точку</p>
      <svg
        ref={svgRef}
      ...
      >
        <defs>
          <pattern id="mpGrid" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
            <path d="M50,0 L0,0 L0,50" fill="none" stroke="rgba(212,168,67,0.05)" strokeWidth="0.5"/>
          </pattern>
        </defs>
        <rect x={VB_X} y={VB_Y} width={VB_W} height={VB_H} fill="url(#mpGrid)" />
        <g transform={KZ_TRANSFORM}>
          <path d={KZ_MAIN} fill="rgba(13,18,34,0.8)" stroke="rgba(0,196,167,0.5)" strokeWidth="15" strokeLinejoin="round" />
        </g>
        {x != null && y != null && (
          <g>
            <circle cx={x} cy={y} r={14} fill="rgba(212,168,67,0.2)" />
            <circle cx={x} cy={y} r={7} fill="#d4a843" stroke="rgba(255,255,255,0.4)" strokeWidth="1.5" />
          </g>
        )}
      </svg>
      {x != null && y != null && (
        <p className={styles.coords}>x_map: {x} · y_map: {y}</p>
      )}
    </div>
  );
}
