...
import styles from './MapCalibrator.module.css';

/* ── Same constants as KazakhstanMap ─────────────────────────────────────── */
const VB_X = -100, VB_Y = 30, VB_W = 1200, VB_H = 820;
const KZ_TRANSFORM = 'translate(-50,874) scale(0.1,-0.1)';
...
export default function MapCalibrator({ projects }: Props) {
  ...
  const [positions, setPositions] = useState<Positions>(() => {
    ...
    for (const p of projects) {
      init[p.id] = { x: p.x_map ?? 500, y: p.y_map ?? 500 };
    }
    return init;
  });
  ...
  const toSVGCoords = useCallback((e: React.MouseEvent) => {
    ...
    const x = Math.round(VB_X + ((e.clientX - rect.left) / rect.width) * VB_W);
    const y = Math.round(VB_Y + ((e.clientY - rect.top) / rect.height) * VB_H);
    return { x, y };
  }, []);
  ...
  const handleMouseMove = useCallback((e: React.MouseEvent<SVGSVGElement>) => {
    ...
    if (!coords) return;
    setPositions(prev => ({ ...prev, [draggingId]: coords }));
  }, [draggingId, toSVGCoords]);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (json.ok) {
        setSaved(true);
        if (!json.supabase) {
          alert('Supabase не настроен.\nИспользуйте «Скопировать значения» и вставьте в data.ts');
        }
      }
    } catch {
  ...
  /* Copy values for data.ts */
  const handleCopy = () => {
    const lines = projects.map(p => {
      const pos = positions[p.id];
      return `  // id ${p.id}: ${p.title.slice(0, 40)}\n  x_map: ${pos.x}, y_map: ${pos.y},`;
    }).join('\n\n');
    const text = `/* Вставить в src/lib/data.ts */\n${lines}`;
    ...
    <div className={styles.wrap}>
      ...
      <div className={styles.mapWrap}>
        <svg
          ...
          style={{ cursor: draggingId !== null ? 'grabbing' : 'default', userSelect: 'none' }}
        >
          {/* Grid */}
          <defs>
            <pattern id="calGrid" x="0" y="0" width="50" height="50" patternUnits="userSpaceOnUse">
              <path d="M50,0 L0,0 L0,50" fill="none" stroke="rgba(212,168,67,0.06)" strokeWidth="0.5"/>
            </pattern>
            <filter id="glow">
          ...
          <rect x={VB_X} y={VB_Y} width={VB_W} height={VB_H} fill="url(#calGrid)" />

          {/* Kazakhstan outline */}
          <g transform={KZ_TRANSFORM}>
            <path d={KZ_MAIN} fill="rgba(13,18,34,0.8)" stroke="rgba(0,196,167,0.6)" strokeWidth="15" strokeLinejoin="round" />
            <path d={KZ_LAKE} fill="rgba(13,18,34,0.8)" stroke="none" />
          </g>
          ...
          {projects.map(p => {
            ...
            return (
              <g
                ...
                onMouseDown={e => { e.preventDefault(); setDraggingId(p.id); setSelectedId(p.id); }}
              ...
              >
                ...
                {/* Selection ring */}
                {isSelected && (
                  <circle cx={pos.x} cy={pos.y} r={16} fill="none" stroke={color} strokeWidth="1.5" opacity={0.6} strokeDasharray="4 3" />
                )}
                {/* Glow */}
                <circle cx={pos.x} cy={pos.y} r={isDragging ? 14 : isSelected ? 12 : 10}
                  fill={color} opacity={hasNoCoords ? 0.08 : 0.2} />
                {/* Core */}
                <circle
                  cx={pos.x} cy={pos.y} r={isDragging ? 9 : 7}
                  fill={hasNoCoords ? 'rgba(13,18,34,0.9)' : color}
                  stroke={color}
                  strokeWidth={hasNoCoords ? '1.5' : '1.5'}
                  opacity={hasNoCoords ? 0.5 : 1}
                  strokeDasharray={hasNoCoords ? '3 2' : undefined}
                />
                {/* ID label */}
                <text
                  x={pos.x} y={pos.y - 14}
      ...
      <div className={styles.sidebar}>
        ...
        <div className={styles.sidebarHeader}>
          <h2 className={styles.sidebarTitle}>Точки на карте</h2>
          <span className={styles.sidebarCount}>{projects.length} проектов</span>
        </div>

        {/* Search */}
        <div className={styles.searchWrap}>
          <svg className={styles.searchIcon} viewBox="0 0 16 16" fill="none" width="13" height="13">
        ...
        <div className={styles.filterRow}>
          {(['all', 'completed', 'in-progress', 'planned', 'no-coords'] as StatusFilter[]).map(f => (
            <button
              key={f}
        ...
        <div className={styles.list}>
          {filtered.length === 0 && (
            <div className={styles.listEmpty}>Ничего не найдено</div>
          )}
          {filtered.map(p => {
            const pos = positions[p.id];
            ...
            const isSelected = selectedId === p.id;
            const hasCoords = p.x_map != null;

            return (
              <div
                key={p.id}
                className={`${styles.listRow} ${isActive ? styles.listRowDragging : ''} ${isSelected ? styles.listRowSelected : ''}`}
                onClick={() => setSelectedId(isSelected ? null : p.id)}
              >
                {/* Status dot */}
                <div className={styles.listDot} style={{ background: color, opacity: hasCoords ? 1 : 0.35 }} />

                {/* Info */}
                <div className={styles.listInfo}>
                  <div className={styles.listTitle}>
                    <span className={styles.listId}>#{p.id}</span>
                    {p.title}
                    {p.featured && <span className={styles.listStar}>★</span>}
                  </div>
                  <div className={styles.listMeta}>
                    <span className={styles.listLocation}>
                      <svg viewBox="0 0 10 12" fill="none" width="8" height="10" style={{ flexShrink: 0 }}>
                        <path d="M5 0C3.07 0 1.5 1.57 1.5 3.5c0 2.63 3.5 7 3.5 7s3.5-4.37 3.5-7C8.5 1.57 6.93 0 5 0z"
        ...
        <div className={styles.actions}>
          <button className={styles.btnCopy} onClick={handleCopy}>
            <svg viewBox="0 0 16 16" fill="none" width="13" height="13">
              ...
              <path d="M11 4V3a1 1 0 00-1-1H3a1 1 0 00-1 1v7a1 1 0 001 1h1" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round"/>
            </svg>
            {copied ? '✓ Скопировано!' : 'Скопировать для data.ts'}
          </button>
          <button className={styles.btnSave} onClick={handleSave}>
            <svg viewBox="0 0 16 16" fill="none" width="13" height="13">
              <path d="M13 4l-3-3H3a1 1 0 00-1 1v10a1 1 0 001 1h10a1 1 0 001-1V4z" stroke="currentColor" strokeWidth="1.4" strokeLinejoin="round"/>
              <path d="M10 13V9H6v4M5 3h5v3H5V3z" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
            {saved ? '✓ Сохранено в БД' : 'Сохранить в Supabase'}
          </button>
        </div>
      </div>
    </div>
