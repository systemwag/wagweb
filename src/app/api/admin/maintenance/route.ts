...
import { MaintenanceProjectSchema, parseOr400 } from '@/lib/admin-schemas';

export async function POST(req: Request) {
  const denied = await requireAdmin();
  if (denied) return denied;
  try {
    const parsed = parseOr400(MaintenanceProjectSchema, await req.json());
    if ('error' in parsed) return NextResponse.json({ ok: false, error: parsed.error }, { status: 400 });

    const supabase = createServiceClient();
    const { data, error } = await supabase.from('maintenance_projects').insert([parsed.data]).select().single();
