/**
 * Official WAG triangle mark from assets/logotriangle.svg.
 * Renders as react-pdf <Svg> with a linear gold gradient (top-left → bottom-right).
 *
 * gradientId must be unique per page when multiple marks are rendered in
 * the same document (react-pdf shares the SVG defs namespace).
 */
...
export function WagMark({ width, height, gradientId = 'wagGold', opacity = 1 }: Props) {
  return (
    <Svg width={width} height={height} viewBox="0 0 719.49 635.66">
      <Defs>
        <LinearGradient id={gradientId} x1="0%" y1="0%" x2="100%" y2="100%">
          <Stop offset="0%"   stopColor="#D4A843" stopOpacity={opacity} />
          <Stop offset="50%"  stopColor="#F0C85A" stopOpacity={opacity} />
          <Stop offset="100%" stopColor="#C49A30" stopOpacity={opacity} />
        </LinearGradient>
      </Defs>
      <Path d={WAG_PATH} fill={`url(#${gradientId})`} />
    </Svg>
  );
}
